...
class DVSFlowDataset(Dataset):
    def __init__(self, root, window, train=True, preload=False):
        ...
        if preload:
            predvs = torch.load(os.path.join(self.root, "dvs.data"))
            if train:
                preflow = torch.load(os.path.join(self.root, "flow.data"))
            self.dvs = []
            if train:
                self.flow = []
            for i in range(len(predvs)):
                if predvs[i].shape[0] == self.window+1:
                    self.dvs.append(predvs[i])
                    if train:
                        self.flow.append(preflow[i])
            ...
            for s in set_list:
                for i in range(s[1]):
                    ...
                    if train:
                        ...
                        if os.path.exists(dvs_1_path) and os.path.exists(dvs_2_path):
                            print(dvs_1_path, dvs_2_path)
                            dvs = torch.stack(read_dvs(dvs_1_path, dvs_2_path, 100000000/(self.window+1)))
                            self.dvs.append(dvs)
