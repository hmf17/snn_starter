...
batch_size = 1 # You should modify the batchsize according to different task
time_windows = 5
...
class PWCSNet(nn.Module):
    ...
    def __init__(self, md=4, training=True):
        ...
        od = nd #
        # convx_x is the conv layer for optic flow estimation, the total layers is 4
    ...
    def forward(self,x):
        self.ImageShape = list(x.size())
        self.IamgeHeight = self.ImageShape[-1]
        self.ImageWidth = self.ImageShape[-2]
        # due to conv layer use ceil
        cfg_net = [[16, ceil(self.ImageWidth/2), ceil(self.IamgeHeight/2)],
                   [32, ceil(ceil(self.ImageWidth/2)/2), ceil(ceil(self.IamgeHeight/2)/2)],
                   [64, ceil(ceil(ceil(self.ImageWidth/2)/2)/2), ceil(ceil(ceil(self.IamgeHeight/2)/2)/2)],
                   [96, ceil(ceil(ceil(ceil(self.ImageWidth/2)/2)/2)/2), ceil(ceil(ceil(ceil(self.IamgeHeight/2)/2)/2)/2)],
                   [128, ceil(ceil(ceil(ceil(ceil(self.ImageWidth/2)/2)/2)/2)/2), ceil(ceil(ceil(ceil(ceil(self.IamgeHeight/2)/2)/2)/2)/2)],
                   ]
        im1 = x[:,:,:1,:,:]
        im2 = x[:,:,1:,:,:]
        ...
        ######
        #1
        im11a_mem = im11a_spike =   torch.zeros(batch_size, cfg_net[0][0], cfg_net[0][1], cfg_net[0][2], device=device)
        im11aa_mem = im11aa_spike = torch.zeros(batch_size, cfg_net[0][0], cfg_net[0][1], cfg_net[0][2], device=device)
        im21a_mem = im21a_spike =   torch.zeros(batch_size, cfg_net[0][0], cfg_net[0][1], cfg_net[0][2], device=device)
        im21aa_mem = im21aa_spike = torch.zeros(batch_size, cfg_net[0][0], cfg_net[0][1], cfg_net[0][2], device=device)
        c11_sumspike = c11_mem = c11_spike = torch.zeros(batch_size, cfg_net[0][0], cfg_net[0][1], cfg_net[0][2], device=device)
        c21_sumspike = c21_mem = c21_spike = torch.zeros(batch_size, cfg_net[0][0], cfg_net[0][1], cfg_net[0][2], device=device)

        #2
        im12a_mem = im12a_spike =   torch.zeros(batch_size, cfg_net[1][0], cfg_net[1][1], cfg_net[1][2], device=device)
        im12aa_mem = im12aa_spike = torch.zeros(batch_size, cfg_net[1][0], cfg_net[1][1], cfg_net[1][2], device=device)
        im22a_mem = im22a_spike =   torch.zeros(batch_size, cfg_net[1][0], cfg_net[1][1], cfg_net[1][2], device=device)
        im22aa_mem = im22aa_spike = torch.zeros(batch_size, cfg_net[1][0], cfg_net[1][1], cfg_net[1][2], device=device)
        c12_sumspike = c12_mem = c12_spike = torch.zeros(batch_size, cfg_net[1][0], cfg_net[1][1], cfg_net[1][2], device=device)
        c22_sumspike = c22_mem = c22_spike = torch.zeros(batch_size, cfg_net[1][0], cfg_net[1][1], cfg_net[1][2], device=device)

        #3
        im13a_mem = im13a_spike =   torch.zeros(batch_size, cfg_net[2][0], cfg_net[2][1], cfg_net[2][2], device=device)
        im13aa_mem = im13aa_spike = torch.zeros(batch_size, cfg_net[2][0], cfg_net[2][1], cfg_net[2][2], device=device)
        im23a_mem = im23a_spike =   torch.zeros(batch_size, cfg_net[2][0], cfg_net[2][1], cfg_net[2][2], device=device)
        im23aa_mem = im23aa_spike = torch.zeros(batch_size, cfg_net[2][0], cfg_net[2][1], cfg_net[2][2], device=device)
        c13_sumspike = c13_mem = c13_spike = torch.zeros(batch_size, cfg_net[2][0], cfg_net[2][1], cfg_net[2][2], device=device)
        c23_sumspike = c23_mem = c23_spike = torch.zeros(batch_size, cfg_net[2][0], cfg_net[2][1], cfg_net[2][2], device=device)

        #4
        im14a_mem = im14a_spike =   torch.zeros(batch_size, cfg_net[3][0], cfg_net[3][1], cfg_net[3][2], device=device)
        im14aa_mem = im14aa_spike = torch.zeros(batch_size, cfg_net[3][0], cfg_net[3][1], cfg_net[3][2], device=device)
        im24a_mem = im24a_spike =   torch.zeros(batch_size, cfg_net[3][0], cfg_net[3][1], cfg_net[3][2], device=device)
        im24aa_mem = im24aa_spike = torch.zeros(batch_size, cfg_net[3][0], cfg_net[3][1], cfg_net[3][2], device=device)
        c14_sumspike = c14_mem = c14_spike = torch.zeros(batch_size, cfg_net[3][0], cfg_net[3][1], cfg_net[3][2], device=device)
        c24_sumspike = c24_mem = c24_spike = torch.zeros(batch_size, cfg_net[3][0], cfg_net[3][1], cfg_net[3][2], device=device)

        # 5
        im15a_mem = im15a_spike =   torch.zeros(batch_size, cfg_net[4][0], cfg_net[4][1], cfg_net[4][2], device=device)
        im15aa_mem = im15aa_spike = torch.zeros(batch_size, cfg_net[4][0], cfg_net[4][1], cfg_net[4][2], device=device)
        im25a_mem = im25a_spike =   torch.zeros(batch_size, cfg_net[4][0], cfg_net[4][1], cfg_net[4][2], device=device)
        im25aa_mem = im25aa_spike = torch.zeros(batch_size, cfg_net[4][0], cfg_net[4][1], cfg_net[4][2], device=device)
        c15_sumspike = c15_mem = c15_spike = torch.zeros(batch_size, cfg_net[4][0], cfg_net[4][1], cfg_net[4][2], device=device)
        c25_sumspike = c25_mem = c25_spike = torch.zeros(batch_size, cfg_net[4][0], cfg_net[4][1], cfg_net[4][2], device=device)
        ...
        x = torch.cat((self.conv5_0(corr5), x),1)
        ...
        ZeroLeftPadding = nn.ZeroPad2d(padding=(1, 0, 0, 0))    # use zeropadding to fit the input image size
